...
$friends_images =Friend::getFriendsImages($guserid,'wallajax');
									...
									<ul class="friendlist">
										...
										<li>
											<div class="friend-list">
												<div class="imgholder">
                                                                                                    <?php $frnd_img = $this->getimage($friendinfo['_id'],'photo'); ?>
					  ...
					  <div class="white-holderbox user_photos panel-shadow">
						...
						<div class="user_destination white-holderbox panel-shadow">
						   ...
						   <div class="clear"></div>
							...
							<div class="friends">
								<?php $likes = Like::getUserPostLike($guserid);
									if(count($likes)>0){
										$lctr = 0;
								?>
									<div class="likes-row">
								...
								<form enctype="multipart/form-data" name="imageForm">
									...
									<div class="tb-user-post-bottom clearfix">
										<div class="tb-user-icon" style="display:none"> 
											<label class="myLabel">
												<!-- uplaod image -->
	<input type="file" id="imageFile1" name="imageFile1[]" required="" data-class="#userwallid #image-holder .img-row" multiple="true"/>
        <input type="hidden" id="counter" name="counter">
												<span aria-hidden="true" class="glyphicon glyphicon-camera tb-icon-fix001"></span>
											</label>
											<a href="javascript:void(0)" class="tb-icon-fix001 add-tagg"><span aria-hidden="true"  class="glyphicon glyphicon-user"></span></a>

											<a href="javascript:void(0)" class="tb-icon-fix001 add-locc"><span aria-hidden="true" class="glyphicon glyphicon-map-marker"></span></a>
											<a href="javascript:void(0)" class="tb-icon-fix001 add-titlee"><span aria-hidden="true" class="glyphicon glyphicon-text-size"></span></a> 
										</div>
...
<script language="javascript"> 
   ...
   function close_all_edit(){
	...
	jq("#walllanguage").select2().val([<?php echo $inter_str; ?>]).trigger("change");
...
<script type="text/javascript">
	...
	function showDate(date)
        ...
        var postBtnEle = jq("#post");
	var HIDE = "HIDE";
	var SHOW = "SHOW";
        function toggleAbilityPostButtonWall(action)
        {
            if(action == "SHOW"){
                    jq(postBtnEle).attr("disabled",false);
            }else{
                    jq(postBtnEle).attr("disabled",true);
            }
	}
	...
	$(".add-locc").click(function(e)
        {
            openLocPost(this);
	});

	$(".add-tagg").click(function(e)
        {
            openLocTag(this);
	});

	$(".add-titlee").click(function(e)
        {
            openTitle(this);
	});
